...
class CoreDataStack: NSObject {
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = Bundle.main.url(forResource: "SeamDemo", withExtension: "momd")!
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        do {
            self.seamStore = try coordinator.addPersistentStore(ofType: SeamStoreType, configurationName: nil, at: url, options: nil) as? SMStore
        } catch {
            // Report any error we got.
            var dict = [String: AnyObject]()
            dict[NSLocalizedDescriptionKey] = NSString(string: "Failed to initialize the application's saved data")
            dict[NSLocalizedFailureReasonErrorKey] = NSString(string: failureReason)
